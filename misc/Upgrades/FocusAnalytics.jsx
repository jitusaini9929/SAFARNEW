...
// ─── MOCK DATA (swap for focusService.getStats()) ─────────────
const MOCK_STATS = {
  totalFocusMinutes: 1340,
  totalBreakMinutes: 290,
  totalSessions: 48,
  ...
  todayFocusMinutes: 86,
  weeklyFocus:  [45, 90, 120, 60, 100, 30, 0],
  weeklyBreaks: [10, 20,  25, 15,  20,  5, 0],
  hourlyDistribution: [
    0,0,0,0,0,0,
    5,10,30,60,75,55,
    40,20,15,50,60,45,
  ...
  avgSessionWithTask: 38,
  avgSessionWithoutTask: 19,
  tasksCompleted: 22,
  recentSessions: [
    { id:"1", startedAt:new Date(Date.now()-1*3600000).toISOString(),  durationMinutes:45, actualMinutes:45, completed:true,  taskText:"Algebra Revision Ch.4" },
    { id:"2", startedAt:new Date(Date.now()-3*3600000).toISOString(),  durationMinutes:5,  actualMinutes:5,  completed:true,  taskText:null },
    { id:"3", startedAt:new Date(Date.now()-4*3600000).toISOString(),  durationMinutes:25, actualMinutes:25, completed:true,  taskText:"Research Paper Outline" },
    { id:"4", startedAt:new Date(Date.now()-26*3600000).toISOString(), durationMinutes:50, actualMinutes:18, completed:false, taskText:"Problem Set 4" },
    { id:"5", startedAt:new Date(Date.now()-27*3600000).toISOString(), durationMinutes:25, actualMinutes:25, completed:true,  taskText:"Revise Lecture Notes" },
    { id:"6", startedAt:new Date(Date.now()-50*3600000).toISOString(), durationMinutes:30, actualMinutes:30, completed:true,  taskText:null },
  ],
};
...
const HOUR_LABELS = { 0:"12am", 6:"6am", 12:"12pm", 17:"6pm", 23:"11pm" };

// ─── ICON HELPERS (inline SVG — no lucide needed) ─────────────
...
const Icons = {
  ...
  Check:      ({color}) => <Icon d="M20 6L9 17l-5-5" color={color||"currentColor"}/>,
  X:          ({color}) => <Icon d="M18 6L6 18M6 6l12 12" color={color||"currentColor"}/>,
...
// ─── STAT CARD ────────────────────────────────────────────────
const StatCard = ({ icon, label, value, sub, accent=T.teal700, bg=T.teal50 }) => (
  <div style={{
...
// ─── WEEKLY CHART (stacked, dynamic scale) ────────────────────
const WeeklyChart = ({ focusData, breakData }) => {
  const totals  = focusData.map((f,i) => f + breakData[i]);
...
// ─── TASK CORRELATION ─────────────────────────────────────────
const TaskCorrelation = ({ withTask, withoutTask }) => {
  const max      = Math.max(withTask, withoutTask, 1);
  const pctBetter= withoutTask > 0
    ? Math.round(((withTask - withoutTask) / withoutTask) * 100) : 100;

  const Bar = ({ val, color, label, sub }) => (
    <div style={{marginBottom:14}}>
      <div style={{display:"flex", justifyContent:"space-between", marginBottom:5}}>
        <span style={{fontSize:12, color:T.slate700, fontWeight:600}}>{label}</span>
        <span style={{fontSize:12, color, fontWeight:800}}>{fmtMins(val)} avg</span>
      </div>
      <div style={{height:8, background:T.slate100, borderRadius:10, overflow:"hidden"}}>
        <div style={{height:"100%", width:`${(val/max)*100}%`, background:color, borderRadius:10, transition:"width 0.6s ease"}}/>
      </div>
      <div style={{fontSize:10, color:T.slate400, marginTop:4}}>{sub}</div>
    </div>
  );

  return (
    <div>
      <Bar val={withTask} color={T.teal700} label="📝 With a task set" sub="Clear goal drives longer sessions"/>
      <Bar val={withoutTask} color={T.slate400} label="🎲 Without a task" sub="Open-ended, shorter sessions"/>
      {withTask > withoutTask && (
        <div style={{background:T.teal50, border:`1px solid ${T.teal100}`, borderRadius:10, padding:"8px 12px", fontSize:11, color:T.teal700, fontWeight:600, display:"flex", alignItems:"center", gap:6}}>
          🎯 Setting a task makes your sessions <b style={{margin:"0 2px"}}>{pctBetter}% longer</b> on average
        </div>
      )}
    </div>
  );
...
const SessionHistory = ({ sessions }) => {
  const focus = sessions.filter((_, i) => i < 6);
  return (
    <div>
      {focus.map((s, i) => {
        const pct = Math.round((s.actualMinutes / s.durationMinutes) * 100);
        return (
          <div key={s.id} style={{
            display:"flex", alignItems:"center", gap:12, padding:"11px 4px",
            borderBottom: i < focus.length-1 ? `1px solid ${T.slate50}` : "none",
          }}>
            <div style={{
...
export default function FocusAnalytics({ onBack } = {}) {
  ...
  useEffect(() => {
    // ↓ Replace with: focusService.getStats().then(setStats).finally(...)
    const t = setTimeout(() => { setStats(MOCK_STATS); setLoading(false); }, 500);
  ...
  const {
    ...
    focusStreak, bestStreak, dailyGoalMinutes, dailyGoalProgress, todayFocusMinutes,
    weeklyFocus, weeklyBreaks, hourlyDistribution,
    avgSessionWithTask, avgSessionWithoutTask, tasksCompleted, recentSessions,
  } = stats;
    ...
    <div style={{minHeight:"100vh", background:T.slate50, fontFamily:"'Plus Jakarta Sans',sans-serif", color:T.slate800}}>
      ...
      <div style={{maxWidth:1060, margin:"0 auto", padding:"28px 20px"}}>
        ...
        <div style={{display:"flex", alignItems:"center", gap:14, marginBottom:26}}>
          {onBack && (
            <button onClick={onBack} style={{width:40, height:40, borderRadius:12, background:T.white, border:`1px solid ${T.slate200}`, display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer"}}>
              <Icons.ArrowLeft/>
            </button>
          )}
          <div style={{flex:1}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:20}}>
          ...
          <div style={{background:T.white, border:`1px solid ${T.slate200}`, borderRadius:20, padding:22, boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
            ...
            <WeeklyChart focusData={weeklyFocus} breakData={weeklyBreaks}/>
            <div style={{marginTop:14, paddingTop:12, borderTop:`1px solid ${T.slate100}`, display:"flex", justifyContent:"space-between"}}>
              ...
              <span style={{fontSize:13,fontWeight:800,color:T.teal700}}>{fmtMins(weeklyFocus.reduce((a,b)=>a+b,0))} focus</span>
            </div>
          </div>
        ...
        <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:20, marginBottom:20}}>
          ...
          <div style={{background:T.white, border:`1px solid ${T.slate200}`, borderRadius:20, padding:22, boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
            <div style={{display:"flex", alignItems:"center", gap:8, marginBottom:4}}>
              ...
              <h3 style={{fontSize:14, fontWeight:700, color:T.slate800, margin:0}}>Task Correlation</h3>
            </div>
            <p style={{fontSize:11, color:T.slate400, margin:"0 0 18px"}}>How task-setting affects session length</p>
            <TaskCorrelation withTask={avgSessionWithTask} withoutTask={avgSessionWithoutTask}/>
            <div style={{marginTop:14, paddingTop:12, borderTop:`1px solid ${T.slate100}`, display:"flex", justifyContent:"space-between"}}>
              <span style={{fontSize:12,color:T.slate500}}>Tasks completed via timer</span>
              <span style={{fontSize:13,fontWeight:800,color:T.teal700}}>{tasksCompleted}</span>
            </div>
          </div>
          ...
          <div style={{background:T.white, border:`1px solid ${T.slate200}`, borderRadius:20, padding:22, boxShadow:"0 1px 4px rgba(0,0,0,0.04)"}}>
            ...
            <div style={{
              ...
              {completionRate>=80
                ...
                : "💡 Setting a task before starting significantly reduces abandonment."}
            </div>
          </div>
        </div>
