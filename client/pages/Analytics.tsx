...
export default function Analytics() {
    ...
    const earnedAchievements = achievements.filter((item) => item.earned).slice(0, 3);

    const executiveCards = report ? [
        {
            label: "Consistency",
            val: `${report.executiveSummary.consistencyScore}%`,
            msg: `${report.executiveSummary.consistentDays}/${report.executiveSummary.evaluationDays} days had meaningful activity.`,
            icon: Zap,
            color: "text-amber-500",
            bg: "bg-amber-500/10",
        },
        {
            label: "Manual Goals Done",
            val: `${report.executiveSummary.goalsCompleted}`,
            msg: `${report.executiveSummary.completionRate}% of ${report.executiveSummary.goalsCreated} planned manual goals were completed.`,
            icon: Target,
        ...
        {
            label: "Focus Time",
            val: formatMinutes(report.executiveSummary.totalFocusMinutes),
            msg: report.executiveSummary.focusMessage,
            icon: Brain,
            color: "text-blue-500",
            bg: "bg-blue-500/10",
        },
        {
            ...
            val: `${report.executiveSummary.reflectionDays}`,
            msg: `${report.executiveSummary.checkInDays} check-in days and ${report.executiveSummary.journalDays} journal days.`,
            icon: Heart,
            color: "text-rose-500",
            bg: "bg-rose-500/10",
        },
                    ...
                    <header className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                        <div className="space-y-2">
                            <div className="flex items-center gap-3 text-emerald-500 font-black text-[10px] uppercase tracking-[0.3em] pl-1">
                                <Sparkles size={14} /> Nishtha Scorecard
                            </div>
                            <h1 className="text-4xl md:text-5xl font-black tracking-tight flex items-center gap-4">
                                <div className="p-3 bg-primary rounded-[20px] shadow-xl shadow-primary/20">
                    ...
                    {loading ? (
                        ...
                        <div className="bg-card border-2 border-dashed rounded-[40px] p-20 text-center space-y-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-muted-foreground font-medium">
                                    Generate the scorecard to pull your current goals, focus sessions, check-ins, and journal activity.
                                </p>
                            </div>
                            <button
                                onClick={handleGenerate}
                        ...
                        <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-700">
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
                                {executiveCards.map((stat, index) => (
                                    <div key={index} className="p-8 bg-card border-2 rounded-[32px] shadow-sm relative overflow-hidden group">
                                ...
                                <div className="lg:col-span-12 xl:col-span-7 bg-card border-2 rounded-[40px] p-8 lg:p-12 shadow-sm overflow-hidden flex flex-col">
                                    <div className="flex items-center justify-between mb-10">
                                        <h3 className="text-xl font-black flex items-center gap-3">
                                            <div className="w-1.5 h-6 bg-primary rounded-full" /> Monthly balance map
                                        </h3>
                                        <span className="text-[10px] font-black text-muted-foreground uppercase opacity-50">
                                            Goals, focus, reflection
                                        </span>
                                    </div>
                                ...
                                <div className="lg:col-span-12 xl:col-span-5 space-y-6">
                                    ...
                                    <div className="bg-card border-2 rounded-[40px] p-8 shadow-sm space-y-6">
                                        ...
                                        <div className="space-y-4">
                                            {[
                                                { label: "Peak focus hour", msg: report.insights.powerHour.message, icon: Zap, color: "text-amber-500" },
                                                { label: "Mood and output", msg: report.insights.moodConnection.message, icon: Heart, color: "text-rose-500" },
                                                { label: "Toughest goal day", msg: report.insights.sundayScaries.message, icon: Brain, color: "text-blue-500" },
                                            ].map((insight, index) => (
                                                <div key={index} className="group p-5 bg-muted/20 border-2 border-transparent hover:border-primary/10 rounded-2xl transition-all">
                            ...
                            <div className="bg-gradient-to-br from-primary/5 to-teal-500/5 border-2 border-primary/10 rounded-[40px] p-10 shadow-sm overflow-hidden relative group">
                                ...
                                <div className="relative z-10">
                                    <div className="flex items-center justify-between mb-8">
                                        <h3 className="text-xl font-black flex items-center gap-3">Achievements</h3>
                                        <button
                                            onClick={() => navigate("/achievements")}
                                            className="text-[10px] font-black uppercase tracking-[0.2em] text-primary hover:gap-2 transition-all flex items-center gap-1"
                                        >
                                            Collect Rewards <ChevronRight size={12} />
                                        </button>
                                    </div>
                                    {earnedAchievements.length === 0 ? (
                                        <div className="py-10 text-center space-y-3">
                                            ...
                                            <p className="text-sm font-bold text-muted-foreground">
                                                Unlock your first achievement soon.
                                            </p>
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                            {earnedAchievements.map((badge, index) => (
                                                <div key={index} className="p-6 rounded-[28px] border-2 bg-card border-primary/20 shadow-lg transition-all flex items-center gap-4 hover:scale-[1.02]">
