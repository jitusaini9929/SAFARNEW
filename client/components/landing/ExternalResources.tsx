        ...
        <section className="bg-slate-50 dark:bg-midnight px-4 md:px-12 py-12 md:py-20 relative z-10">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-4xl mx-auto">
                    {/* YouTube Channel */}
                    <a href="https://youtube.com/@safarparmar?si=Mvs6U5JaSGojIzSM" target="_blank" rel="noopener noreferrer" className="group bg-white dark:bg-[#1E293B] rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
                        <div className="relative aspect-video bg-gradient-to-br from-red-900/20 to-red-600/20 overflow-hidden">
                            ...
                            <div className="absolute bottom-4 left-4 right-4 z-10">
                                ...
                                <p className="text-white/90 text-xs drop-shadow-lg">{t('resources.yt_channel')}</p>
                            </div>
                        </div>
                        <div className="p-6">
                            <h4 className="font-bold text-slate-900 dark:text-white mb-2">{t('resources.yt_heading')}</h4>
                            <p className="text-slate-600 dark:text-slate-400 text-sm mb-3">{t('resources.yt_desc')}</p>
                    ...
                    {/* Course Link */}
                    <a href="https://www.parmaracademy.in/" target="_blank" rel="noopener noreferrer" className="group bg-white dark:bg-[#1E293B] rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all hover:-translate-y-1">
                        <div className="relative aspect-video bg-gradient-to-br from-pink-900/20 to-pink-600/20 overflow-hidden">
                            <img loading="lazy" src={courseImg} alt="Parmar Academy Course" className="w-full h-full object-contain bg-white p-4 group-hover:scale-105 transition-transform duration-300" />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
                            <div className="absolute bottom-4 left-4 right-4 z-10">
                                <p className="text-white font-semibold text-sm drop-shadow-lg">Parmar Academy</p>
                                <p className="text-white/90 text-xs drop-shadow-lg">{t('resources.course_label')}</p>
                            </div>
                        </div>
                        <div className="p-6">
                            <h4 className="font-bold text-slate-900 dark:text-white mb-2">{t('resources.course_heading')}</h4>
                            <p className="text-slate-600 dark:text-slate-400 text-sm mb-3">{t('resources.course_desc')}</p>
