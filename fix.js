...
content = content.replace(regex, function (match) {
    return match
        ...
        .replace(/className="flex items-center gap-2 bg-white\/10 backdrop-blur-md border border-white\/20 rounded-full px-3 py-2"/, 'className={`flex items-center gap-2 backdrop-blur-md rounded-full px-3 py-2 ${showAnalytics ? "bg-slate-200/50 dark:bg-slate-800/50 border border-slate-300 dark:border-white/20" : "bg-white/10 border border-white/20"}`}')
        .replace(/className="p-1\.5 rounded-full hover:bg-white\/10 transition-colors"/g, 'className={`p-1.5 rounded-full transition-colors ${showAnalytics ? "hover:bg-slate-300/50 dark:hover:bg-slate-700/50 text-slate-700 dark:text-white" : "hover:bg-white/10 text-white"}`}')
        ...
        .replace(/className="rounded-xl h-\[52px\] w-\[52px\] p-0 hover:bg-white\/10 border border-white\/20 bg-white\/10 backdrop-blur-md"/, 'className={`rounded-xl h-[52px] w-[52px] p-0 border backdrop-blur-md ${showAnalytics ? "bg-slate-200/50 dark:bg-slate-800/50 border-slate-300 dark:border-white/20 hover:bg-slate-300/50 dark:hover:bg-slate-700/50" : "bg-white/10 border-white/20 hover:bg-white/10"}`}')
        .replace(/className="h-\[52px\] w-\[52px\] rounded-lg border-2 border-white shadow-sm"/, 'className={`h-[52px] w-[52px] rounded-lg border-2 shadow-sm ${showAnalytics ? "border-white dark:border-slate-800" : "border-white"}`}');
});
